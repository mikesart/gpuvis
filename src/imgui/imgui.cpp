...
void ImGui::NewFrame()
{
    ...
    if (g.MovingWindowMoveId && g.MovingWindowMoveId == g.ActiveId)
    {
        ...
        if (g.IO.MouseDown[0])
        {
            ...
            if (actual_moved_window->PosFloat.x != pos.x || actual_moved_window->PosFloat.y != pos.y)
                MarkIniSettingsDirty(actual_moved_window);
            actual_moved_window->PosFloat = pos;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (flags & ImGuiWindowFlags_ChildWindow)
        {
            ...
            if (!(flags & ImGuiWindowFlags_Popup) && !window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow))
